...
class Rational
{
    public:
        ...
        friend std::istream& operator >>(std::istream &is, const Rational& frac);
...
void Rational::common_denominator(Rational cd[], 
                                  const Rational& a, 
                                  const Rational& b)
{
    ...
    // Fucking hackfixes everywhere to prevent the copy constructor 
    // from firing and simplifying us.
    cd[0] = a_;
    cd[1] = b_;
}
...
// Added from Fraction class (Mike Hentrich, September 2019)
// but removed "const" before Rational&
...
std::istream& Rational::read(std::istream &is)
{
  std::cout << "\n\nEnter Rational Number as fraction a/b,\n"
	...
	while ( (line[i]) && (line[i] != '/') )
	{
		...
		{
			this->m_numerator = 10*m_numerator + (line[i] - '0');
			i++;
		}
		else
	...
	while (line[i])
	{
		if (line[i]>= '0' && line[i] <='9')
		{
			this->m_denominator = 10*m_denominator + (line[i] - '0');
			i++;
		}
		else
			throw "You must type a number";
    ...
    if (negs == 0) this->m_negative = false;
    else {
        if (negs%2 == 1) this->m_negative = true;
        else this->m_negative = false;
    }
	return is;
}
